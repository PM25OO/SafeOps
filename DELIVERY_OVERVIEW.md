...
1. `STB-2.1~2.3`：稳定性加固（上下文失效容错、请求弹性、状态文案统一）
2. `TEST-2.1~2.2`：API + 前端集成测试增强（不引入浏览器自动化）
3. `AUD-2.1~2.4`：审计持久化升级（DB主路径 + JSONL备份）
4. `UI-2.0~2.2`：悬浮球二级按钮触发 + 轻量 SidePanel 只读信息面板回归
